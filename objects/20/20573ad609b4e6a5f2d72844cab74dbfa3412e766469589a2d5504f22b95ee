    ...
    try:
        ...
        for c in commits:
            for cs in c.get("changesets", []):
                for bk in ("blob_before", "blob_after"):
                    bh = cs.get(bk)
...
def cmd_offer(args):
    ...
    try:
        ...
        commits = db.query(conn, "SELECT * FROM repo_commits WHERE repo_id = %s AND rev > %s ORDER BY rev ASC", (config["repo_id"], canonical_latest_rev))
        if not commits:
