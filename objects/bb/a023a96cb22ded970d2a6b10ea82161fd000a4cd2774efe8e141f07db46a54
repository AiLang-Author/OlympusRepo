...
def commit(conn, repo_id: int, user_id: int, message: str,
    ...
    # Store new/modified blobs (idempotent — safe even if we retry)
    for filepath, new_hash in changes["added"]:
        full_path = os.path.join(repo_root, filepath)
        objects.store_file(full_path, objects_dir)

    for filepath, old_hash, new_hash in changes["modified"]:
        full_path = os.path.join(repo_root, filepath)
        objects.store_file(full_path, objects_dir)

    # Build tree entries from index + changes
    index = worktree.load_index(repo_root)
    tree_entries = dict(index)

    for filepath, new_hash in changes["added"]:
        full_path = os.path.join(repo_root, filepath)
        stat = os.stat(full_path)
        tree_entries[filepath] = {
            "hash": new_hash,
            "mtime": stat.st_mtime,
            "size": stat.st_size,
        }
    ...
    for filepath, old_hash in changes["deleted"]:
        tree_entries.pop(filepath, None)

    # Tree hash = hash of sorted entries
    tree_content = json.dumps(
        {k: v["hash"] for k, v in sorted(tree_entries.items())},
        sort_keys=True
    ).encode()
